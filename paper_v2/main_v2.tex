...
corrects v1 (Version note). Code, probes and per-recording
...
\input{../agg/table_delta_full.tex}
\end{table*}
...
\label{sec:repro}
Code, operators, adapters, all twelve probes, per-cell predictions
for every (model, probe, cell), tables and proofs:
...
table and figure regenerates from the released per-recording
predictions without a GPU; the feature and cell caches (about
$12$\,GB) are not archived and regenerate from the released scripts
in a few GPU-hours (Appendix~E).
